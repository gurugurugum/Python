...
def QMC_move(config, ann_para):
    ...
    # あるトロッタ数cで、時刻a,bにいる都市p,qをマーク
    p = config[c][a].index(1)
    q = config[c][b].index(1)

    # costの初期化
    cost = costc = costq_1 = costq_2 = costq_3 = costq_4 = 0

    # CLASSICAL COST
    for j in range(NCITY):
        l_p_j = distance(POINT[p], POINT[j])/max_distance
        l_q_j = distance(POINT[q], POINT[j])/max_distance
        costc += 2*(-l_p_j*config[c][a][p] - l_q_j*config[c][a][q])*(config[c][a-1][j]+config[c][(a+1)%TOTAL_TIME][j])+2*(-l_p_j*config[c][b][p] - l_q_j*config[c][b][q])*(config[c][b-1][j]+config[c][(b+1)%TOTAL_TIME][j])

    # QUANTUM COST
    para = (1/BETA)*math.log(math.cosh(BETA*ann_para/TROTTER_DIM)/math.sinh(BETA*ann_para/TROTTER_DIM))
    costq_1 = config[c][a][p]*(config[(c-1)%TROTTER_DIM][a][p]+config[(c+1)%TROTTER_DIM][a][p])
    costq_2 = config[c][a][q]*(config[(c-1)%TROTTER_DIM][a][q]+config[(c+1)%TROTTER_DIM][a][q])
    costq_3 = config[c][b][p]*(config[(c-1)%TROTTER_DIM][b][p]+config[(c+1)%TROTTER_DIM][b][p])
    costq_4 = config[c][b][q]*(config[(c-1)%TROTTER_DIM][b][q]+config[(c+1)%TROTTER_DIM][b][q])

    # Total Cost
    cost = costc/TROTTER_DIM+para*(costq_1+costq_2+costq_3+costq_4)

    # 確率min(1,exp(-BETA*cost))でflip
    if cost <= 0:
        config[c][a][p]*=-1
        config[c][a][q]*=-1
        config[c][b][p]*=-1
        config[c][b][q]*=-1
    elif np.random.random() < np.exp(-BETA*cost):
        config[c][a][p]*=-1
        config[c][a][q]*=-1
        config[c][b][p]*=-1
        config[c][b][q]*=-1
